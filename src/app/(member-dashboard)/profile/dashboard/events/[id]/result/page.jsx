...
export default async function ParticipantResultPage({ params }) {
    ...
    const data = await getEventParticipantResult(user?.id, param?.id)
    if (data?.error || !data) {
        throw Error()
    }
                        ...
                        <CardDescription>
                            <div className="flex flex-wrap gap-y-2 gap-x-4 mt-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>{event.type === "OFFLINE" ? event?.location : "Online"}</span>
                                </div>
                            </div>
                        </CardDescription>
                    </CardHeader>
                    ...
                    <Card className="md:col-span-1">
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex flex-col items-center text-center">
                                ...
                                {data.complete ? (
                                    <Badge className="mt-2 bg-green-500/10 text-green-500 border-green-500/20">Completed</Badge>
                                ) : (
                            ...
                            <div className="space-y-2">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        {data.score ? (
                                            <span
                                                className={`font-bold text-xl ${data.score >= 80
                                                    ? "text-green-500"
                                                    : data.score >= 60
                                                        ? "text-yellow-500"
                                                        : "text-red-500"
                                            ...
                                            <span className="font-bold text-xl text-red-500">Failed</span>
                                        )}
                                    </div>
                                    {data.score ? (
                                        <Progress
                                            value={data.score}
                                            className={`h-2 ${data.score >= 80
                                                ? "bg-green-500"
                                                : data.score >= 60
                                                    ? "bg-yellow-500"
                                                    : "bg-red-500"
                                ...
                                {data.feedback ? (
                                    ...
                                    <div className="text-center py-4 text-muted-foreground">No feedback submitted</div>
                                )}
                            </div>
                        </CardContent>
                    </Card>
