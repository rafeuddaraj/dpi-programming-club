...
export default function Lessons({
  ...
  useEffect(() => {
    (async () => {
      const resp = await updateLastModuleAndLesson(workshopParticipantId, {
      ...
      {allLessons?.map((lesson) => (
        <div
          ...
          className={`flex items-center gap-2 p-2 rounded-md cursor-pointer ${
            currentLessonId === lesson?.id ? "bg-muted" : "hover:bg-muted/50"
          } ${lesson?.status === "Upcoming" ? "cursor-not-allowed" : ""}`}
          onClick={
            lesson?.status !== "Upcoming"
              ? () => handleNextLesson(lesson)
              : () => {}
          }
        >
          ...
          <div className="flex flex-col">
            <span className="text-sm">
              {lesson.name}{" "}
              {lesson?.status !== "Completed" && (
                <Badge className={lesson?.statusClasses}>
                  {lesson?.status}
                </Badge>
              )}
            </span>
