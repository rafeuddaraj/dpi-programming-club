    ...
    >
          ...
          <div className="mt-4 justify-between items-center">
            <div className="flex flex-col">
              ...
              <span className="text-sm text-gray-800">
                {lesson.startingDate
                  ? format(new Date(lesson.startingDate), "PPP hh:mm aa")
                  : "Not set"}
              </span>
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-sm text-gray-800">
                {lesson.endingDate
                  ? format(new Date(lesson.endingDate), "PPP hh:mm aa")
                  : "Not set"}
              </span>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" className="w-full">
          <Link href={`/dashboard/workshops/${lesson?.id}/assignment`}>
            Assignment
          </Link>
        </Button>
        <Button variant="ghost" size="icon" className="w-full">
          <Link
            href={`/dashboard/workshops/${lesson?.id}/quizzes?lessonId=${lesson?.id}`}
          >
            Quiz
          </Link>
        </Button>
        <Badge
          variant={lesson.isActive ? "outline" : "destructive"}
          className="text-xs"
        >
