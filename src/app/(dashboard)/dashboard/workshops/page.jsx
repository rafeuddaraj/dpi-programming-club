...
export default async function WorkshopsPage({ searchParams: searchParam }) {
  ...
  const resp = await getWorkshops(query, activeStatus, filterType)
      ...
      <Tabs defaultValue="grid" className="w-full">
        ...
        <TabsContent value="grid" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {workshopsData?.length ? workshopsData?.map((workshop) => (
              <Card key={workshop.id} className="overflow-hidden">
                ...
                <CardContent>
                  <div className="flex items-center gap-2 mb-3">
                    <Avatar className="h-8 w-8">
                      <AvatarImage src="/placeholder.svg?height=32&width=32" alt={workshop.instructor} />
                ...
                <CardFooter className="flex justify-between border-t pt-3">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {workshop.type === "OFFLINE" && `${workshop.totalSeats} seats`}
                  </div>
                </CardFooter>
              </Card>
            )) : <>Not found</>}
        ...
        <TabsContent value="list" className="mt-6">
          <div className="rounded-md border">
            {workshopsData?.length ? <><div className="grid grid-cols-12 p-4 font-medium border-b">
              ...
              {workshopsData.map((workshop) => (
                <div key={workshop.id} className="grid grid-cols-12 p-4 items-center border-b last:border-b-0">
                  <div className="col-span-4">
                    <div className="font-medium">{workshop.name}</div>
                    <div className="text-sm text-muted-foreground">by {workshop.instructor}</div>
                  </div>
