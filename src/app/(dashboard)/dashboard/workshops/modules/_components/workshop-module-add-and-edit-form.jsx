...
export default function WorkshopModuleAddAndEditForm({ workshop, moduleData }) {
  ...
  const onSubmit = async (data) => {
    try {
      const resp = moduleData
        ? await updateWorkshopModule({ workshopModuleId: moduleData?.id, data })
        : await createWorkshopModule({ workshopId, data });
      if (resp?.error) {
        throw Error();
      }
      toast.success(
        `Workshop module ${moduleData ? "update" : "create"} successfully.`
      );
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-3xl font-bold">
          {workshop ? "Edit " : "Create "} Module
        </h1>
      </div>

      {workshop && (
        <div className="text-muted-foreground">
          {moduleData ? "Updating " : "Adding "} module to workshop:{" "}
